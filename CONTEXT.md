...
- **Hard-fact eligibility** — an Item may create an Incident only with evidence: a CVE id, or `ai_relevant=true` plus a non-Unknown vendor. Ineligible Items stay in the **Wire** (pipeline-only; unattached Items not displayed on the site).
